...
test.group('Collect Metrics middleware', () => {
    ...
    test('Should group same status code when enabled', async ({ assert, client }) => {
        ...
        const response = await client.get('/metrics');
        assert.include(response.text(), 'adonis_http_request_durations_count{method="GET",url="/test",statusCode="4xx"} 3');
        assert.include(response.text(), 'adonis_http_request_durations_count{method="GET",url="/test",statusCode="2xx"} 2');
        cleanup();
    });
    ...
    test('should include query params if specified', async ({ assert, client }) => {
        ...
        const response = await client.get('/metrics');
        assert.include(response.text(), 'adonis_http_request_durations_count{method="GET",url="/test?status=200&foo=bar",statusCode="2xx"} 1');
        assert.include(response.text(), 'adonis_http_request_durations_count{method="GET",url="/test?status=201&foo=baz",statusCode="2xx"} 1');
        cleanup();
    });
